import { fetchUserData, ExpenseType, updateExpense} from "@/lib/features/userSlice";
...
export default function UpdateExp({
  ...
  const item = storeSelectedItem as ExpenseType;
  ...
  const [sendValues , setSendValues] = useState({
    ...
    amount : 0,
    date : new Date,
    _id : (storeSelectedItem as ExpenseType)._id,
  });

  const handleClick = async () =>{
    await dispatch(updateExpense(sendValues))
        ...
        >
          <div className="w-full h-[85%] flex flex-col justify-center items-center bg-red-700">
            <div className="flex flex-col w-[90%] h-[30%] justify-center items-start">
              ...
              <input onChange={(e)=>setSendValues({...sendValues , category : e.target.value})}
                className="w-full outline-1 outline-gray-300 selection:outline-blue-500 selection:outline-1 p-3"
                type="text"
                name="source"
                placeholder={(storeSelectedItem as ExpenseType).category}
              />
            </div>
            ...
            <div className="flex flex-col w-[90%] h-[30%] justify-center items-start">
              ...
              <input onChange={(e)=>setSendValues({...sendValues , amount : Number(e.target.value)})}
                ...
                type="number"
                name="amount"
                placeholder={((storeSelectedItem as ExpenseType).amount).toString()}
              />
            </div>
